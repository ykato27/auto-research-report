...
2. `prompts/talent_mgmt_weekly.md` に従い、Claude が Web 検索でニュースを収集・公開日検証・選定・深掘り分析
3. 7〜10件に絞り込み、各記事に150-200字の解説・直接引用・Skillnoteへの示唆・顧客への示唆を付与
4. `reports/talent_mgmt_weekly_YYYYMMDD.txt` を Git push
5. GitHub Actions が Teams へ配信

プロンプト実行時は `knowledge/skillnote_context.md` を参照し、Skillnote固有の製品・事業情報に引き寄せた示唆を生成します。
...
├── prompts/
│   └── talent_mgmt_weekly.md      # 人材・スキル戦略週次まとめ（v7: 7-10件深掘り形式）
├── knowledge/
│   └── skillnote_context.md       # Skillnote固有のプロダクト・事業知識（示唆生成に使用）
├── .github/
│   └── workflows/
...
- v7形式（`## N. タイトル` + `**日付**:` + `参考: URL`）および v6形式（`【カテゴリ】` + `（公開日:）` + `（URL:）`）を自動判別して解析
- 公開日が対象期間外、未来日、不明の場合は失敗
- URL内の日付が対象期間より古い場合は失敗
- SEO統計まとめ等のblocked domainを含む場合は失敗
- フッターの `今週のトピック数：N件` と記事数が一致しない場合は失敗
- 7件未満は警告扱い
...
Claude Code Web が起動
    ↓
knowledge/skillnote_context.md を読み込み
    ↓
10クエリ並列 Web 検索 → 公開日検証 → 7〜10件に選定
    ↓
各記事：150-200字解説 + 直接引用 + Skillnoteへの示唆 + 顧客への示唆
    ↓
reports/talent_mgmt_weekly_YYYYMMDD.txt を main へ push
    ↓
GitHub Actions (send_talent_mgmt_teams.yml) が起動
    ↓
validate_talent_mgmt_report.py で検証（v6/v7自動判別）
    ↓
send_report_to_teams.py で Teams へ配信（記事ごとにセクション分割）
```
